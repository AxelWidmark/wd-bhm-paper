...
\end{equation}
basically an integral over the possible properties of a hypothetical white dwarf drawn from the population model, times the probability of being selected. The selection criteria of our sample is for all objects to have a Gaia $G$ band apparent magnitude brighter than 18. The selection factor is equal to

\begin{equation}
	S(\Teff,\logg,\mathbf{x}) =
    \int_{\text{min}(\hat{m}_G)}^{\text{max}(\hat{m}_G)} \de \hat{m}_G \mathcal{N}\big( \hat{m}_G | m_G(\Teff,\logg,\mathbf{x}),\sigma_{\hat{m}_G}(m_G(\Teff,\logg,\mathbf{x})) \big),
\end{equation}
which is the probability that a hypothetical object with the given parameters is observed to have an apparent Gaia $G$ band magnitude in the allowed range. Because the error $\sigma_{\hat{m}_g}$ varies with magnitude (typically larger error for a dimmer object), this quantity is in principle a function the true apparent magnitude. However, because the errors on the $G$ band magnitude are small we will neglect them, and the selection factor is just a Heaviside function, like

\begin{equation}
	S(\Teff,\logg,\mathbf{x}) =
    \Theta \big( 18-m_G(\Teff,\logg,\mathbf{x}) \big).
\end{equation}
